...
extension HTTPHeader {
    ...
    ///
    /// For a Basic `Authorization` header use
    ...
    ///
    /// `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not
...
extension HTTPHeaders {
    /// The default set of `HTTPHeaders`. Includes `Accept-Encoding`, `Accept-Language`, and
    /// `User-Agent`.
