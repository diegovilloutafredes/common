...
extension UIImage {
    ...
    public func averageColor(algorithm: AverageColorAlgorithm = .simple) -> UIColor? {
        ...
        let pixelBufferPointer = pixelBuffer.bindMemory(to: UInt32.self, capacity: width * height)
        ...
        for x in 0 ..< width {
            ...
            for y in 0 ..< height {
                ...
                let offset = y * width
                let index = x + offset
                let pixel = pixelBufferPointer[index]
        ...
        return .init(red: averageRed / 255.0, green: averageGreen / 255.0, blue: averageBlue / 255.0, alpha: 1.0)
    }
    ...
    private func red(for pixelData: UInt32) -> UInt8 {
        ...
        //  ^alpha   ^red     ^blue    ^green
        //
        // The alpha got pulled over which we don't want or care about, so we need to get rid of it. We can do that with the bitwise AND operator (&) which compares bits and the only keeps a 1 if both bits being compared are 1s.
        // So we're basically using it as a gate to only let the bits we want through. 255 (below) is the value we're using as in binary it's 11111111 (or in 32 bit,
        // it's 00000000 00000000 00000000 11111111) and the result of the bitwise operation is then:
        //
        // 00000000 00000000 11111111 11111111
        ...
        .init((pixelData >> 16) & 255)
    }

    private func green(for pixelData: UInt32) -> UInt8 { .init((pixelData >> 8) & 255) }
    private func blue(for pixelData: UInt32) -> UInt8 { .init((pixelData >> 0) & 255) }
}
