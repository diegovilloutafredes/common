...
| A9 | 🟡 | **Extra intermediate UIView in every VC.** `loadView()` creates a `container` UIView, adds `mainView` to it, sets `self.view = container`. Every screen carries an extra UIView in the hierarchy. `self.view = mainView` achieves the same result with one fewer node. |
...
| C6 | 🟡 | 30 `nonisolated(unsafe)` usages total. The two mutable-state cases (C1, C2) are fixed. Remaining usages are UInt8 associated-object keys (address-only, never mutated — safe) and `NSCache`-backed formatters (thread-safe by design). | Various |
...
| U2 | 🟠 | **`UIViewBuilder.buildOptional` returns a hidden `UIView()` placeholder** when the optional is nil. Even with `isHidden = true`, this adds a real node to the view hierarchy with associated memory, layout calculations, and hit-testing overhead. Returning `nil` from `buildOptional` and using `buildBlock` overloads that accept `UIView?` would eliminate the placeholder. |
